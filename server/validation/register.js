...
module.exports = function registerInputValidation(data) {
  ...
  if(!Validator.isLength(data.first_name, { min: 2, max: 30 })) {
    error.first_name = 'This field must have a length between 2 and 30 characters.';
  }
  ...
  if(!Validator.isLength(data.last_name, { min: 2, max: 30 })) {
    error.last_name = 'This field must have a length between 2 and 30 characters.';
  }
  ...
  if(Validator.isLowercase(data.password) || Validator.isUppercase(data.password)) {
    error.password = 'Password must contain upper and lowercase letters.';
  }
  // check if password has valid length
  if(!Validator.isLength(data.password, { min: 8, max: 30 })) {
    error.password = 'This field must have a length between 8 and 30 characters.';
  }
